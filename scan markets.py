...
from data_collection import polymarket, binance, bybit
...
def scan_one(asset, timeframe, correlated_prices):
    ...
    try:
        ...
        oi_hist = []
    try:
        bybit_raw = bybit.get_funding_rate_history(symbol, limit=30)
        bybit_hist = [float(f["fundingRate"]) for f in reversed(bybit_raw)]  # Bybit returns newest-first
    except Exception:
        bybit_hist = []
    ...
    context = {
        ...
        "binance_funding_history": funding_hist,
        "bybit_funding_history": bybit_hist,
